...
export default async (data: {
    ...
    /// 将
    copyFileSync(
        pathJoin(nowDir, 'assets/temporary.ico'),
...
function createDir() {
    printSome('开始添加库目录：'.concat(name).concat('/src'));
    /** 创建库地址 */
    mkdirSync(name + '/src');
    printSome('开始添加库 tools 地址：'.concat(name).concat('/src/tools'));
    mkdirSync(name + '/src/tools');
    /** 创建测试文件地址 */
    printSome('开始添加库测试目录：'.concat(name).concat('/static'));
    mkdirSync(name + '/static');
    printSome(
        '开始添加库测试 ico 文件放置目录：'.concat(name).concat('/public'),
    );
    mkdirSync(name + '/public');
    // mkdirSync(name + '/static/route');
}
